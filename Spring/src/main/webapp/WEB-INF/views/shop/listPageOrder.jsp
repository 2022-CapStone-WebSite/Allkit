...
<div id = "root">
	...
	<section id="container">
		...
		<div id="container_box">
			...
			<section id="content">
				 ...
				 <ul class="orderList">
				  <c:forEach items="${listPageOrder}" var="listPageOrder">
				  <li>
				  <div>
				   <p><span>주문번호</span><a href="/shop/orderView?n=${listPageOrder.orderId}">${listPageOrder.orderId}</a></p>
				   <p><span>수령인</span>${listPageOrder.orderRec}</p>
				   <p><span>주소</span>(${listPageOrder.userAddr1}) ${listPageOrder.userAddr2} ${listPageOrder.userAddr3}</p>
				   <p><span>가격</span><fmt:formatNumber pattern="###,###,###" value="${listPageOrder.amount}" /> 원</p>
				   <p><span>상태</span>${listPageOrder.delivery }</p>
				  </div>
				  </li>
				  </c:forEach>
				 </ul>
 ...
 <ul class="pagination">
  <c:if test="${pageMaker.prev}">
	<li><a href="listPageOrder${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
  </c:if> 
  
  <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
   <li <c:out value="${pageMaker.cri.page == idx? 'class=active' : '' }"/>> 
   <a href="listPageOrder${pageMaker.makeQuery(idx)}">${idx}</a>
   </li>
  </c:forEach>
    
  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
   <li><a href="listPageOrder${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
  </c:if> 
 </ul>
</div> 
